...
const {AdoptionRequest, UserRole, User, Cat, CatUser} = require("../../models")
const emailService = require("../services/emailService")
const emailServ = require("../services/emailService")

const sendAdoptionRequest = async (status, adoptionRequest, sender, receiver, cat, userAddress) => {
    await User.sequelize.transaction(async (t) => {
        ...
        for (const otherAdoptionRequest of otherAdoptionRequests) {
            await AdoptionRequest.update({status: 'declined'}, {where: {id: otherAdoptionRequest.id}, transaction: t})
            ...
            const sender = await User.findOne({where: {id: otherUserRole.userId}})
            await emailServ.sendDeclineAdoption(sender, receiver, cat)
        }

        await Cat.update({ownerId: sender.id}, {where: {id: adoptionRequest.catId}, transaction: t})
        const catUser = await CatUser.findOne({where: {catId: adoptionRequest.catId}}, {transaction: t})
        if (catUser) {
            await CatUser.update({ownerId: sender.id}, {where: {id: catUser.id}, transaction: t})
        }
        await AdoptionRequest.update({status, addressId: userAddress.id}, {where: {id: adoptionRequest.id}, transaction: t})
        ...
        if (adoptionRequest.status === 'pending') {
            ...
            for (let userAdoptionRequest of userAdoptionRequests) {
                if (userAdoptionRequest.role === 'sender') {
                    const sender = await User.findOne({where: {id: userAdoptionRequest.userId}})
                    await emailService.sendDeclineAdoption(sender, receiver, cat)
                }
            }
            await UserRole.destroy({where: {adoptionRequestId: adoptionRequest.id}})
...
const deleteAdoptionRequest = async (userAdoptionRequest, userAdoptionRequests, adoptionRequestId, res) => {
    let visibleCount = 0
    ...
    if (visibleCount === 1) {
        ...
        await userAdoptionRequest.update({isVisible: !userAdoptionRequest.isVisible})
        return res.status(200).json({status: 'AdoptionRequest deleted successfully'})
    }
}
...
const deleteAdoptionRequestUser = async (user) => {
    ...
    for (let userAdoptionRequest of userAdoptionRequests) {
        if (userAdoptionRequest.role === 'receiver') {
            const adoptionRequest = await AdoptionRequest.findOne({where: {id: userAdoptionRequest.adoptionRequestId}})
            if (adoptionRequest && adoptionRequest.status === 'pending') {
                const cat = await Cat.findOne({where: {id: adoptionRequest.catId}})
                const senderUserRoles = await UserRole.findAll({where: {adoptionRequestId: adoptionRequest.id, role: 'sender'}})
                for (let senderUserRole of senderUserRoles) {
                    const sender = await User.findOne({where: {id: senderUserRole.userId}})
        ...
        const adoptionRequest = await AdoptionRequest.findOne({where: {id: userAdoptionRequest.adoptionRequestId}})
        const otherUserRoles = await UserRole.findAll({where: {adoptionRequestId: adoptionRequest.id}})
...
module.exports = {sendAdoptionRequest, updateEmail, deleteAdoptionRequestCat, deleteAdoptionRequest, deleteAdoptionRequestUser}