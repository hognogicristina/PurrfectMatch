...
const catExistValidator = async (req, res) => {
    ...
    const favorite = await Favorite.findOne({where: {userId: req.user.id, catId: req.params.id}})
    ...
    if (cat.userId === req.user.id) {
        return res.status(400).json({error: 'You cannot favorite a cat you are guardian of'})
        ...
        return res.status(400).json({error: 'You cannot favorite a cat that is already adopted'})
    }
