    ...
    if (req.params.id) {
        ...
        if (req.query.selectedBreed) {
            ...
            if (breeds.includes(req.query.selectedBreed)) {
                const breedExists = cats.some(cat => cat.breed === req.query.selectedBreed)
                if (!breedExists) {
                    return res.status(404).json({error: 'No Result Found'})
                }
            } else {
        ...
        if (req.query.selectedGender) {
            if (req.query.selectedGender === 'Male' || req.query.selectedGender === 'Female') {
                const genderExists = cats.some(cat => cat.gender === req.query.selectedGender)
                if (!genderExists) {
                    return res.status(404).json({error: 'No Result Found'})
                }
            } else {
        ...
        if (req.query.selectedNoHealthProblem !== undefined) {
            const noHealthProblemExists = cats.some(cat => cat.healthProblem === null)
