...
async function sendActivationEmail(user) {
    ...
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'activationEmail.pug'), {filename: 'activationEmail.pug'})
...
async function sendResetEmail(user) {
    const {link, sender} = await emailHelper.generateTokenAndSignature(user, 'activation')
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'resetEmail.pug'), {filename: 'resetEmail.pug'})
...
async function sendConfirmationEmail(user) {
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'confirmationEmail.pug'), {filename: 'confirmationEmail.pug'})
...
async function sendDeclineAdoption(sender, receiver, cat) {
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'declineAdoption.pug'), {filename: 'declineAdoption.pug'})
    const mailOptions = {
        from: receiver.email,
        to: sender.email,
...
async function sendDeleteAccount(user) {
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'deleteAccount.pug'), {filename: 'deleteAccount.pug'})
    const sender = await User.findOne({where: {role: 'admin'}})
    const mailOptions = {
        from: sender.email,
        to: user.email,
...
async function sendResetPassword(user) {
    ...
    const compiledFunction = pug.compileFile(path.join(__dirname, '..', 'templates', 'resetPassword.pug'), {filename: 'resetPassword.pug'})
    const mailOptions = {
        from: sender.email,
        to: user.email,
