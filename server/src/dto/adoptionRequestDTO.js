...
const {Image, Address, Cat, AdoptionRequest, UserRole, } = require('../../models')
...
async function transformAdoptionRequestToDTO(adoptionRequest, user) {
    ...
    const formattedDate = moment(adoptionRequest.createdAt).format('YYYY-MM-DD HH:mm:ss')
...
async function transformAdoptionRequestsToDTO(user, sortOrder) {
    const userAdoptionRequests = await UserRole.findAll({ where: { userId: user.id, isVisible: true } })
    ...
    for (let userAdoptionRequest of userAdoptionRequests) {
        ...
        if (diffWeeks <= 1) {
            ...
            formattedDate = moment(adoptionRequest.createdAt).format('YYYY-MM-DD HH:mm:ss')
        }
...
module.exports = {transformAdoptionRequestToDTO, transformAdoptionRequestsToDTO}
